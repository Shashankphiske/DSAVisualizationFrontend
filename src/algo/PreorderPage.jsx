...
const PreorderPage = () => {
  ...
  const fetchPreorderSteps = async (adj) => {
  ...
  const handlePlay = async () => {
    ...
    if (steps.length === 0) {
      try {
        ...
        const backendSteps = await fetchPreorderSteps(adj);
  ...
  useEffect(() => {
    if (!isPlaying || currentStepIndex >= steps.length) {
      if (currentStepIndex >= steps.length && steps.length > 0) {
        setExplanation(`Preorder traversal complete! Result: [${steps.join(", ")}]`);
      }
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <div className="max-w-4xl mx-auto bg-gray-800 p-6 rounded mb-6">
        <h2 className="text-xl font-semibold mb-2">About Preorder Traversal</h2>
      ...
      <div className="max-w-5xl mx-auto bg-gray-800 p-8 rounded">
        <h3 className="text-xl font-semibold mb-6 text-center">Binary Tree - Preorder Traversal (Root → Left → Right)</h3>
